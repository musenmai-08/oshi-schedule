...
- API integration (Vitest + Supertest): auth、招待外、登録/重複/上限、登録直後の同期成功/延期/失敗と登録保持、即時再試行の冪等性、共有snapshotのユーザー別Calendar展開、pause/resume、削除、rate limit、所有権、account deletion を in-memory store/Fake gateway で観察する。
- E2E (Playwright): `NEXT_PUBLIC_DEMO_MODE=true` と Fake APIを専用Web/API port（既定3310/4310）で毎回起動し、既存serverは再利用しない。固有health identity確認後、dashboard→検索→登録・初回同期→停止/再開→手動同期→削除を行う。登録モーダルの安全なエラー表示はAPI応答を差し替えて確認する。実 Google/YouTube へは送らない。並列CIはjobごとに`E2E_WEB_PORT/E2E_API_PORT`を割り当てる。
