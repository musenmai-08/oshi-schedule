...
- API integration (Vitest + Supertest): auth、招待外、登録/重複/上限、初回job 201/dispatch失敗、manual 202/active dedupe、status lifecycle/所有権、atomic claim/duplicate/stale recovery、共有snapshotのユーザー別Calendar展開、pause/resume、削除、proxy spoof/rate limit、account deletionをin-memory store/Fake gatewayで観察する。
- E2E (Playwright): `NEXT_PUBLIC_DEMO_MODE=true`とFake APIを専用port（既定3310/4310）で毎回起動する。dashboard→検索→登録→QUEUED/RUNNING polling→初回成功→停止/再開→manual 202/polling→削除を確認し、実Google/YouTubeへは送らない。
