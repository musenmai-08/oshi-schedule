...
Critical/High修正では、固定Clockとfault injectionを使い、削除timeout・段階再開・stale writer、過去配信のmapped/unmapped分岐、LIVEの仮終了超過、Calendar 404、決定的event ID、lease fencing、同期履歴/runId、OAuth backoff/Retry-After、暗号鍵tamper/rotation、quota unit/予約/retry/page/date境界/cache/延期を回帰検証する。MySQLでは期限切れlease takeoverと30並列quota予約を実transactionで検査する。E2EはFake auth/YouTube/Calendarを利用し外部APIを呼ばない。実資格情報の受入はstaging checklistとして別管理し、最新件数と実行結果は[再監査修正台帳](../reviews/reaudit-remediation.md)へ記録する。
