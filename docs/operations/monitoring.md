...
| HTTP API log     | request ID、route key、status、integration status/latency、source IP    | staging 14日、production 30日 |
| RDS log          | error、slow query（必要時）                                             | staging 14日、production 30日 |
| CloudTrail       | Secret access、IAM、resource/config変更                                 | AWS標準と監査方針に従う       |

禁止するログ項目はaccess/refresh token、OAuth code、Secret、API key、`DATABASE_URL`、暗号文、暗号鍵、email、Supabase user ID、Calendar IDである。structured loggerのallowlistを使い、body/header/envの一括dumpをしない。HTTP API access logはAuthorization、query、bodyを含めない。
...
| ------------- | ------------------------------------------------------- | ----------------------------------------- | -------------------------------------------- |
| API 5xx       | HTTP API `5xx`                                          | 5分で5件以上                              | SNS email、logをrequest IDで追跡             |
| API latency   | HTTP API integration latency p95                        | 15分で5秒超                               | DB/job dispatch latencyを分離                |
| API discovery | VPC Link state、Cloud Map healthy instance              | unavailable/0 instance                    | ECS health、SG、service discoveryを確認      |
...
| sync dispatch | SQS age/DLQ、Pipe execution、targeted task exit         | DLQ 1件、QUEUED/RUNNING stale             | RunTask受付とapplication失敗を区別           |
...
CDKはAPI CPU、HTTP API 5xx、RDS free storage、worker exit非0、sync job DLQのalarmとSNS、月額forecast 80%のBudget通知を作成する。stagingのBudget既定値は25 USD（forecast通知点20 USD）、productionは75 USDである。Budgetはhard limitではない。SyncRun stale、Pipe execution、再認証数、quota残量、worker heartbeatの追加alarmはstaging baseline後の課題とし、高cardinality IDをmetric dimensionに使わない。

## worker監視

定期EventBridge Schedulerはflexible window off、maximum event age 1時間、retry 2回、SQS DLQで定義する。要求時同期は別のencrypted Standard SQS + DLQ + EventBridge Pipeを使う。taskが起動後exit 1になった場合はScheduler/PipeのRunTask成功とは別なので、ECS Task State Change eventのcontainer exit code非0をEventBridge ruleでSNSへ送る。初回deploy時は定期scheduleをdisabledとする。
...
- HTTP API request/4xx/5xx/integration latency、VPC Link、Cloud Map instance
- sync queue age/DLQ、Pipe execution、targeted task exit
