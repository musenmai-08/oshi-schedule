...
2026-09-06にserverless preview最終受入を完了した。staging専用 `Backup staging serverless database` workflowをGitHub OIDCのrepository immutable-ID subjectへ修正し、backup roleはapp dump objectの`PutObject`/`GetObject`とmigration Secret読み取りだけを許可する。既存backup bucketを再利用し、workflowはOIDC、`pg_dump`、manifest検証、S3保存まで成功した。objectはSSE-S3、public access block有効、lifecycle 7日である。取得したdumpを一時PostgreSQLへrestoreし、app schema 12 tableの復元を確認後、containerとdumpを破棄した。Prisma migration metadataはapp-schema dumpの対象外である。
...
2026-09-06にproduction preflight blocker 2〜6をコード上で解消した。GitHub OIDCはimmutable owner/repository IDに固定し、`production-infra` / `production-migration` / `production-amplify` / `production-backup`の別承認境界と最小権限roleをIaC化した。Amplifyは`detached`でAppだけを作成し、guard付きrepository接続後の`connected`でBranch→Domainを作る。migration workflowはinfra deployから分離し、同一commitの成功attestationがdeployの必須条件である。production backupはapp dumpとGit commit/migration ID/checksum manifestを対にし、restore先の`app._prisma_migrations`まで機械照合する。修正後のfull detached diffはCREATE 46、UPDATE/DELETE/REPLACE 0、Branch/Domain 0、旧resource/staging参照0である（未作成DB Secret 2件は非実在ARNによる構造確認）。bootstrap-only diffはinfra/migration Role+PolicyのCREATE 4だけだった。AWS/Supabase/Google writeは0で、role/Appの実作成、DB境界、baseline、外部公開gateは未実施である。
...
2026-09-06にproduction DB bootstrapを実施した。bootstrap-only CloudFormation updateはproduction infra/migration GitHub OIDC Role各1件とinline Policy各1件だけをCREATEし、stackは`UPDATE_COMPLETE`、post-diffは0である。既存baseline table 12件と`public._prisma_migrations`の1件のchecksumがrepository baselineと一致したため、baselineを再適用せずmetadataだけを非破壊に`app._prisma_migrations`へ移動した。Prisma migration statusはup-to-dateである。既存`oshi_runtime`はCSPRNG passwordへ更新し、transaction-mode runtime URL Secretを作成した。roleは非superuser、role/database CREATE不可、app schema CREATE不可、業務12 tableのDMLのみでmigration metadata DMLを持たず、runtime connectionも成功した。temporary attestationはcurrent commitとmigration ID/checksumで生成・検証した。正式production contextのfull detached diffはCREATE 42、UPDATE/DELETE/REPLACE 0、RDS/ECS/VPC/VPC Link/Cloud Map/Pipe/Public IPv4/staging runtime参照0、Amplify Branch/Domain 0である。production Lambda/API/Amplify/Scheduler/backupは未deploy・未実行である。

2026-09-06にproduction detached deploy前のLambda bundling失敗を修正した。clean checkoutでは`@oshi-schedule/shared`のpackage exportが存在しない`packages/shared/dist/index.js`を指す一方、API/Worker Lambdaはsource-first bundleであるためesbuildが解決できなかった。Lambda bundlingのworkspace aliasに`@oshi-schedule/shared`→`packages/shared/src/index.ts`を追加し、既存API runtime/lambda-env aliasと同じsource-first契約へ統一した。APIとWorkerの両entry pointがshared sourceを取り込み、API/sharedの`dist`を入力にしない回帰testを追加した。shared `dist`を一時退避した実CDK synthも成功した。修正commitはrelease SHAを変更するため、旧`9169f54`のproduction-migration attestationは次回deploy gateに使用できない。新SHAでCI成功後に`production-migration`を一度実行し、新しいattestationを入力にfull detached deployを再開する。

2026-09-06のproduction detached deploy後、Lambda CodeのCDK差分2件を調査した。API/Workerの実行コード内容はlocal bundleと一致していたが、GitHub LinuxのPrisma `native`（debian）query engineがzipに追加され、macOS localには存在しないためasset hashだけが異なっていた。bundling後にRHEL用engine以外の`libquery_engine-*`を削除する処理へ修正し、同一source/lockfileのproduction synthを2回実行してAPI/Worker asset hashが一致すること、bundle内engineがRHEL用1件だけになることを確認した。次回はこの修正SHAでCI、production-migration attestation、detached deployを順に再実施する。既存の3c23218 attestationは修正後SHAへ再利用しない。

2026-09-06にclean checkoutのWeb testも同じgenerated `@oshi-schedule/shared/dist` exportをNode解決して失敗することを修正した。Turboの`test`から`^build`依存を外し、Web/API/WorkerのVitestを`@oshi-schedule/shared` source aliasへ統一した。Workerは既存Lambda source-first aliasと同じAPI runtime/lambda-env source aliasも使用する。package exportは変更していないため、Next.js・Node.js・production buildの配布物契約は維持される。shared distを退避し、Turbo cacheを無効化した`pnpm test`、Lambda bundle/CDK synth、typecheck/lintが成功した。今回のcommitでrelease SHAは再び変わるため、`cdff67b`を含む既存production-migration attestationは次回deploy gateに使用せず、新SHAのCI成功後にprotected workflowを一度実行する。

2026-09-06にproduction detached deploy run `34031121693`（release SHA `3c23218ca95ddc1cc0523ad5a87e3f9a091465b6`）の成功を確認した。CloudFormationは`UPDATE_COMPLETE`、API/Worker LambdaはActive、API health/readyはHTTP 200、未認証保護APIは401、Queue/DLQは0、ESMはEnabled（BatchSize 1 / MaximumConcurrency 2）、Schedulerは`DISABLED`、production alarmsは全件`OK`だった。AmplifyはAppのみ（repository未接続、Branch/Domain 0）で、API custom domainとACMは正常である。正しいproduction `schema=app` URLでPrisma migration statusはup-to-date、runtime接続も成功した。旧RDS/ECS/VPC/VPC Link/Cloud Map/Pipe/Public IPv4はproduction stackに存在せず、staging/legacy stackの更新はない。post-deployのローカルCDK diffにはLambda Code更新2件が残り完全なdiff 0ではないため、bundle再現性の確認を次工程のblockerとして扱う。AWS read-only確認以外の追加writeは行っていない。

2026-09-06に修正SHA `a9a5cf1ce4d425e7519d3fe0c17a00cf24415feb`でproduction detached redeployが成功した。GitHubのmigration run `34033791236`とdeploy run `34034352733`はいずれも成功し、同SHAのattestation gateを通過した。CloudFormationは`UPDATE_COMPLETE`で、変更はAPI/Worker Lambda Code更新のみ。LambdaはActive、health/readyは200、未認証APIは401、Queue/DLQ 0、ESM Enabled（1/2）、alarms OK、Scheduler DISABLED、Prisma migration up-to-date、runtime接続成功を確認した。production Amplify App `oshi-schedule-production-web`（App ID `d1v67c1ruct5nd`）はrepository未接続、Branch/Domain 0のdetached状態である。次はユーザーがGitHub接続を完了した後、connected phaseでBranch→Domainを作成し、初回buildへ進む。

Amplify repository接続の次工程は、GitHub Environment `production-amplify` に保護ルール（必須reviewer）を設定し、Variables `AWS_REGION=ap-northeast-1`、`PRODUCTION_AMPLIFY_CONNECTOR_ROLE_ARN=arn:aws:iam::741448960817:role/oshi-schedule-production-github-amplify-connect`、`PRODUCTION_AMPLIFY_APP_ID=d1v67c1ruct5nd`を登録し、短命PATをSecret `AMPLIFY_GITHUB_PAT`へ直接登録してworkflow `Connect production Amplify repository`を一度だけ実行する手順である。PATは対象リポジトリ`musenmai-08/oshi-schedule`だけへ限定し、接続完了後に破棄する。workflowは既存Appのrepositoryだけを更新し、Branch/Domainは作成しない。接続後に`before-connected` preflightを通し、別承認のconnected deployでmain Branch→DomainをIaCから作成する。

repository接続workflowは当初`amplify:ListApps`を呼んだため、対象App限定のconnector RoleでAccessDeniedとなり、`UpdateApp`前に停止した。`AMPLIFY_APP_ID`を必須入力として`GetApp`、`ListBranches`、`ListDomainAssociations`だけを使うようpreflightを修正した。既存connector policyには同じApp ARNへのこれら3 read APIと`UpdateApp`がすでにあり、IAM権限の追加・拡大は不要である。失敗runはrequest file作成・`UpdateApp`の前に終了したためPATはAWSへ渡されていない。次回はこの修正SHAのCI成功後、既存の短期PATが有効なら同Environment Secretを一度だけ再利用でき、期限切れ・期限不明なら再発行して成功確認後に破棄する。

その後の接続再試行では、実Amplify serviceが`ListBranches`を`apps/d1v67c1ruct5nd/branches/*`として評価し、App ARNだけのpolicyを拒否した。Service Authorization ReferenceのApp ARN記載と異なる実評価のため、`ListBranches`にはApp ARNと同App配下`branches/*`、`ListDomainAssociations`にはApp ARNと同App配下`domains/*`を併記する。`GetApp`/`UpdateApp`はApp ARNだけに維持し、`Resource: "*"`やaccount-wide listは追加しない。`UpdateApp`前に止まったため、PATは引き続きAmplifyへ未送信である。

2026-09-09にproduction connected phase後のpost-deploy監査を完了した。対象release SHA `a95655d3782e784ebf6125fa098acf6df8802dd8` のmigration run、Amplify接続run、deploy runはいずれも成功し、CloudFormationは`UPDATE_COMPLETE`だった。Amplify App `d1v67c1ruct5nd` はGitHub repositoryへ接続済みで、`main` Branch 1件、`oshi-schedule.com` Domain Association 1件（`AVAILABLE`、main関連付け）、初回job `1`のBUILD/DEPLOY/VERIFYはすべて`SUCCEED`した。公開Web、API `/health`、`/ready`はHTTP 200、保護APIは401だった。API/Worker LambdaはActive、SQS/DLQは0、ESMはEnabled（BatchSize 1 / MaximumConcurrency 2）、production alarmsはOK、SchedulerはDISABLED、Prisma migrationはup-to-dateである。CloudWatch直近24時間にAPI/WorkerのERROR・secretパターンはなく、RDS/ECS/VPC/VPC Link/Cloud Map/Pipeはproduction stackに存在しない。staging/legacy stackの更新時刻に今回の変更はない。Amplify環境変数はproduction API、production Supabase URL、`WEB_ORIGIN`、demo=falseと一致することを値非表示で確認した。Google/Supabase管理画面の外部allowlistはAWSから独立しており、コード上のproduction callback契約との完全一致はユーザー管理画面で最終確認する。接続に使ったPATのGitHub Environment Secretからの削除/revokeは未確認で、ユーザー作業として必要である。

2026-09-10にproduction Schedulerの受入を一時的な1分間隔で確認したところ、4回連続で2対象中1対象だけが`PARTIAL_FAILED`となった。失敗対象は保存済みrefresh tokenのAES-GCM認証に失敗し、現在設定済みkey ID自体は存在するが復号できない状態だった。このためGoogle token refresh・Calendar API requestは開始されておらず、Calendar APIのHTTP errorではない。Queue/DLQは0、Worker error/throttleとAlarmは正常、もう一方の対象はSUCCESS、mapping重複は0である。Schedulerは`rate(1 hour)`・`DISABLED`へ安全に戻した。cipherのOpenSSL例外を`TOKEN_DECRYPTION_FAILED`へ正規化し、該当ユーザーを`reauthRequired`へ安全に遷移させる修正を次releaseへ含める。対象ユーザーの再同意でcredentialを更新してから、Schedulerを1時間間隔で再受入する。credential/mappingの削除は行わない。
...
2026-09-11にproduction backup workflow run `34603673493`の受入を完了した。S3の同一prefix dump/manifest pairは非0 byte、SSE-S3 (`AES256`)、非公開、7日Lifecycleで、manifestのGit commitとbaseline migration ID/checksumはrepository stateと一致した。productionとは隔離した一時PostgreSQL 17で空の`app` schemaを作成後にrestoreし、`app` schema 13 table、39 index、23 constraint、`app._prisma_migrations`をmanifest/repositoryと機械照合した。Supabase Auth schemaはdump/restoreの対象外である。rehearsal後に一時container/network/downloaded filesを破棄し、production DB、Scheduler（`rate(1 hour)`/`ENABLED`）、Calendar、S3 backup objectは変更していない。以後の日次backup運用は受入済みである。

### 2026-09-11 production release final audit（read-only）

productionの稼働状態を再監査した。AWS account `741448960817` / `ap-northeast-1`、CloudFormation `oshi-schedule-production` は `UPDATE_COMPLETE`。Amplify App `d1v67c1ruct5nd` は repository接続済み、`main` Branch 1件、domain `oshi-schedule.com` は `AVAILABLE`、初回job `1` は成功、公開WebはHTTP 200だった。API `/health`・`/ready` は200、未認証 `/api/v1/me` は401。API/Worker LambdaはActive、ESMはEnabled（BatchSize 1 / MaximumConcurrency 2）、queue/DLQはすべて0、直近24時間のLambda Errors/Throttlesは0、alarmsは全件OKである。

production DBはread-only確認で、ACTIVE subscription 2件・Credential復号可能2件・`reauthRequired=false`・CalendarConnection ACTIVE 2件、直近MANUAL/SCHEDULED SyncRunはともにSUCCESS、PAUSEDの旧subscription `cmtoa49qo0002gy02s4ijoxn7` はScheduler対象外だった。Prisma migration statusはup-to-date。Schedulerは最終設定 `rate(1 hour)` / `ENABLED`、payload `{kind:"scheduled"}`。backup run `34603673493` は成功し、S3 objectはSSE-S3/private/public access block/7日Lifecycleである。production stackにRDS/ECS/VPC/VPC Link/Cloud Map/Pipe/Public IPv4はなく、staging stackの最終更新時刻にも今回のproduction監査による変更はない。Terms/Privacyの運営者、13歳未満、日本法、限定scope、Google user data非広告・非販売・非AI学習方針は実装と一致し、repo内にtoken-looking literalはない。

Google Auth PlatformのAudience `External` / `In production`、Branding公開、Data Access verification不要という状態はユーザーの外部コンソール確認を証跡として記録する。Google Consoleは本環境からread-only API検証できないため、独立した機械確認は未実施である。接続用PATはGitHub Environment SecretおよびGitHub個人設定から削除・revoke済みである。

CloudFormation drift検出（resource非変更のread-only検査）では、API Gateway access-log destination ARNとAmplify root domain prefixのAWS正規化差異2件のみを確認した。SNS/Schedulerのdriftはなく、旧duplicate UserのPAUSED subscription/mappingは削除せずrelease blockerにも含めない。

2026-09-12 production正式受入完了後の状態: `AlertsEmailSubscriptionV3`はCloudFormation `CREATE_COMPLETE`かつConfirmed 1 / Pending 0、Schedulerは`rate(1 hour)` / `ENABLED`、production stackは`UPDATE_COMPLETE`。CloudFormation driftはAmplify root prefixとAPI Gateway access-log ARNの既知AWS正規化差異2件のみで、release blockerではない。`AMPLIFY_GITHUB_PAT` Environment Secretと接続用classic PATは削除・revoke済み。production release blockerは0件で、旧duplicate UserのPAUSED subscription/mapping cleanupだけを任意の別承認作業として残す。
