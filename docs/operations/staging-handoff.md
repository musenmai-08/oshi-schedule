...
2026-08-30にproduction deploy blockerのCI失敗を調査した。Actions APIでは`614a8cd`以前のvalidate/e2e失敗を確認し、ログdownloadは403だった。ローカルのsandboxでは`supertest`が`0.0.0.0`へlistenできないEPERMで失敗するが、sandbox外で同じvalidate commandを実行すると通過したため、これはGitHub Ubuntu runnerの失敗原因ではない。最初に`scripts/aws/staging-context.test.mjs`の旧runtime digest固定を承認済みdigestへ更新したが、修正commit `2c84501`のActions annotationで残存failureを特定した。原因は`infra/test/oshi-schedule-stack.test.ts`のECR-first bootstrap synth testがGitHub Ubuntu runnerで既定5秒を超えることだった。CDK synthとresource assertionの内容は維持し、当該重いtestだけに15秒の明示timeoutを設定した。続いて、Terms/Privacy正式化後もE2Eが廃止済みのデモ文言を待つため、CIではretriesを含めて長時間失敗することを確認した。正式なTerms本文の13歳以上利用条件を検証するよう期待値だけを更新し、正式文面の内容は変更していない。ローカルでinfra test（109件）、typecheck、lint、Playwright E2E（9件）は成功した。ローカルDockerの容量不足によりMySQL integration DBの新規作成はできず、GitHubのvalidate/e2e green確認をproduction投入の最終gateとする。AWS write、deployは行っていない。
