...
2026-08-30にproduction deploy blockerのCI失敗を調査した。Actions APIでは`614a8cd`以前のvalidate/e2e失敗を確認し、ログdownloadは403だった。ローカルのsandboxでは`supertest`が`0.0.0.0`へlistenできないEPERMで失敗するが、sandbox外で同じvalidate commandを実行すると通過したため、これはGitHub Ubuntu runnerの失敗原因ではない。実際のvalidate failureは、`scripts/aws/staging-context.test.mjs`が既にsource of truthへ反映済みのruntime digestではなく旧digestを固定していたことだった。承認済みdigestへ更新後、`pnpm test`（全script testを含む）、`pnpm typecheck`、`pnpm lint`、`pnpm test:e2e`（9件）が成功した。テスト意図やbusiness logicは緩めていない。修正commitのGitHub CIがgreenになることをproduction投入の最終gateとする。AWS write、deployは行っていない。
