...
Google Auth PlatformのAudience `External` / `In production`、Branding公開、Data Access verification不要という状態はユーザーの外部コンソール確認を証跡として記録する。Google Consoleは本環境からread-only API検証できないため、独立した機械確認は未実施である。PATのGitHub Environment/GitHub上でのrevoke状態もAWS/repositoryから判定できず、ユーザー確認を要する。

CloudFormation drift検出（resource非変更のread-only検査）では、(1) SchedulerのIaC期待値`DISABLED`に対する実状態`ENABLED`（正式稼働の意図的差分）、(2) SNS alert subscriptionの欠落、(3) API Gateway access-log destination ARNのAWS正規化差分、(4) Amplify root domain prefixの`""`/`null`表現差分が検出された。公開Web/APIやalarmsは正常だが、SNS通知購読欠落とSchedulerのIaC driftは次回IaC reconcileの残blocker（Medium）として扱う。旧duplicate UserのPAUSED subscription/mappingは削除せず、release blockerにはしない。
