...
2026-08-30に上記production imageのCI Trivy成功とECR Basic Scanの差異を再調査した。ECRで検出した7件は、`perl` source package `5.36.0-7+deb12u3`のCRITICAL 1件（`CVE-2026-12087`）とHIGH 3件（`CVE-2026-48959`、`CVE-2026-48961`、`CVE-2026-7017`）、`openssl` `3.0.20-1~deb12u2`のHIGH 3件（`CVE-2026-54874`、`CVE-2026-63072`、`CVE-2026-63076`）だった。CIはTrivy Actionのcache既定値を使っていたため、キャッシュ済み脆弱性DBを用いる可能性があり、ECRと同時点のDBで評価する保証がなかった。CIとproduction promotion workflowは`cache: 'false'`を明示し、production `.trivyignore`だけを使うfresh DBのHIGH/CRITICAL gateへ固定した。policy validatorもこの契約を検査する。
