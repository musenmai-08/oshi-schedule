...
今回はscopeを変更しない。Supabase Google provider側の許可scope、OAuth同意画面、既存refresh tokenが持つgrantを同時に移行できず、stagingで再同意と全Gateway操作を検証していないためである。現行`calendar`はユーザーがアクセスできる全calendarを対象にできる一方、候補scopeはアプリ作成calendarだけに限定される。STEP 4またはSTEP 7までに、Google Cloud/Supabase設定、既存ユーザーの`prompt=consent`による再同意、保存scopeの更新、作成・取得・event CRUD・calendar削除・削除event復旧をstagingで確認し、OAuth審査資料を限定scopeに合わせる。
